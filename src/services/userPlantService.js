...
export const getUserPlants = async () => {
  const response = await axios.get(`${API_BASE_URL}/userplants`);
  return response.data;
};

// 2. جلب نبتة محددة للمستخدم بواسطة ID
export const getUserPlantById = async (userPlantId) => {
  const response = await axios.get(`${API_BASE_URL}/userplants/${userPlantId}`);
  return response.data;
};

// 3. إضافة نبتة جديدة للمستخدم
export const addUserPlant = async (plantId, plantData = {}) => {
  const response = await axios.post(`${API_BASE_URL}/userplants`, {
    plant_id: plantId,
...
export const updateUserPlant = async (userPlantId, updateData) => {
  const response = await axios.put(`${API_BASE_URL}/userplants/${userPlantId}`, updateData);
  return response.data;
};

// 5. حذف نبتة من قائمة المستخدم
export const deleteUserPlant = async (userPlantId) => {
  const response = await axios.delete(`${API_BASE_URL}/userplants/${userPlantId}`);
  return response.data;
};

// 6. جلب دراسة التربة الموصى بها للنبتة
export const getPlantSoils = async (plantId) => {
  const response = await axios.get(`${API_BASE_URL}/plants/${plantId}/soils`);
  return response.data;
};

// 7. جلب جدول الري (في حال وجود مسار خاص في الباك إند)
export const getWateringSchedule = async (userPlantId) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/userplants/${userPlantId}/watering`);
    return response.data;
  } catch (err) {
...
// 8. جلب جدول التسميد (في حال وجود مسار خاص في الباك إند)
export const getFertilizerSchedule = async (userPlantId) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/userplants/${userPlantId}/fertilizer`);
    return response.data;
  } catch (err) {
